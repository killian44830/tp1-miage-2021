...
Le code n’est pas structuré en couches. On retrouve dans le même package des fichiers liés à la persistance, configuration, REST et entités.

*Où se trouve le code métier (voir la règle de gestion RG 1) ?*

Le code métier se trouve dans le contrôleur. Pour ce petit projet ce n’est pas gênant, cependant cela posera vite des problèmes pour un projet de taille normale.

*Cette règle est-elle facilement testable par un test unitaire ?*

Ce projet n’est pas facilement testable.
...
Cette ligne de commande permet de démarrer un conteneur docker.
L'option -p : Permet de publier un conteneur, et accessible via une translation de port depuis le serveur.
L'option -e : Force Docker a utiliser un driver exec sépcifique.
L'option -it : Permet de garder STDIN ouvert, même si pas attaché tout en allouant un pseudo terminal.
...
TodoItemRepository est null car on a pas créer de endpoint dans la base un client REST. Pour injecter, il faut mieux mettre une annotation @Inject devant la fonction TodoListController()
...
L'architecture hexagonale décompose un système en plusieurs composants interchangeables et faiblement couplés.
